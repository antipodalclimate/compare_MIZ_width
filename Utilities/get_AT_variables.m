function AT_stats = get_AT_variables(IS2_obj)
...
AT_LIF = movsum(seg_len.*is_included,slide_use,'samplepoints',dist) ./ movsum(seg_len,slide_use,'samplepoints',dist);

% SIC is segment length weighted mean
AT_SIC = (1/100)*movsum(seg_len.*conc,slide_use,'samplepoints',dist) ./ movsum(seg_len,slide_use,'samplepoints',dist);

% FSD is segment length ratio
AT_RFSD = movsum(seg_len.^3,slide_use,'samplepoints',dist) ./ movsum(seg_len.^2,slide_use,'samplepoints',dist);
